...
st.set_page_config(page_title="BizCardX: Extracting Business Card Data with OCR | By RAGUL S",
                   ...
                   menu_items={'About': """# This OCR app is created by *RAGUL S*!"""})
st.markdown("<h1 style='text-align: center; color: Green;'>BizCardX: Extracting Business Card Data with OCR</h1>",
            unsafe_allow_html=True)

# SETTING-UP BACKGROUND IMAGE
st.markdown(f""" <style>.stApp {{
                        background:url("file:///C:/Users/Ragul/Downloads/world-map-in-plain-blue-bgi1o5hls0nblaxy.jpg");
...
if selected == "Home":
    ...
    with col1:
        st.image(Image.open(".venv\Data-Extraction-Logo.jpg"), width=500)
...
if selected == "Upload & Modify":
    ...
    if upload is not None:
        ...
        with col2:
            ...
            df = pd.DataFrame(ext_text)
        if selected == "Preview":
            st.subheader(":green[Data's extracted from the image]")
            ...
            with col4:
                Upload = st.button("Upload to SQL Database")
            if Preview:
